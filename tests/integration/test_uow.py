...
def test_uow_can_retrive_at_batch_and_allocate_to_it(session_factory):
    session = session_factory()

    insert_batch(session, "batch1", "WOOD-WORKBENCH", 100, None)
    session.commit()
    ...
    with uow:
        ...
        line = model.OrderLine("o1", "WOOD-WORKBENCH", 10)
        batch.allocate(line)
        uow.commit()
